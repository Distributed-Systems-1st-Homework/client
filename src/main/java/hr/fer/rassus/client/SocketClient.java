...
public class SocketClient implements SocketApi {
    ...
    public void startup() {
        try {
            ...
            this.csvFile = reading.loadCSVFile(Constants.FILE_PATH.getConstant());
            ...
            ClientApplication.logger.error("Server has been shut down");
            ClientApplication.logger.info("Shutting down sensor...");
            System.exit(1);
        }
    }
    ...
    public void loop() {
        try {
            ...
            ResponseEntity response = this.clientToServer.shutdown(this.clientUsername);
            if (response.getStatusCode().is2xxSuccessful()) {
                ClientApplication.logger.info("The sensor has been shut down");
                System.exit(0);
            } else {
                ClientApplication.logger.info("The sensor could not be shut down (deleted on server)");
                System.exit(1);
            }
        }
    }
    ...
    public RestTemplateClient getClientToServer() {
