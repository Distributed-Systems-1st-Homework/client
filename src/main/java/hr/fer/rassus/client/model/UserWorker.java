...
public class UserWorker implements Runnable {
    ...
    public void run() {
        ...
        // Send neighbour sensor stopping message
        try {
            PrintWriter outToOtherSensor = new PrintWriter(new OutputStreamWriter(socket.getOutputStream()));
            outToOtherSensor.print("SENSOR_STOP_MEASURE");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
